...
inline cactus::engine::SpectrogramConfig get_parakeet_spectrogram_config() {
    cactus::engine::SpectrogramConfig cfg{};
    cfg.n_fft        = 512;
    cfg.frame_length = 400;
    cfg.hop_length   = 160;
...
inline cactus::engine::SpectrogramConfig get_htk_spectrogram_config() {
    cactus::engine::SpectrogramConfig cfg{};
    cfg.n_fft        = 321;
...
inline cactus::engine::SpectrogramConfig get_gemma4_audio_spectrogram_config(
...
inline void apply_whisper_v3_overrides(cactus::engine::SpectrogramConfig& cfg) {
    cfg.fft_override = 512;
    cfg.remove_dc_offset = false;
}

inline std::vector<float> compute_spectrogram_graph(
    ...
    int log_mode = 0;
    if (cfg.log_mel) {
        if (std::strcmp(cfg.log_mel, "log") == 0) log_mode = 1;
        else if (std::strcmp(cfg.log_mel, "log10") == 0) log_mode = 2;
        else if (std::strcmp(cfg.log_mel, "dB") == 0) log_mode = 3;
    }
    int pad = (cfg.pad_mode && std::strcmp(cfg.pad_mode, "constant") == 0) ? 1 : 0;

    float effective_floor = cfg.mel_floor_additive ? 0.0f : cfg.mel_floor;
    int effective_log = cfg.mel_floor_additive ? 0 : log_mode;

    CactusGraph g;
    size_t mel_node = g.mel_filter_bank(num_freq_bins, mel_bins,
                                         min_freq, max_freq, sampling_rate,
                                         norm_type, scale_type);

    size_t wav_input = g.input({waveform.size()}, Precision::FP32);
    g.set_input(wav_input, waveform.data(), Precision::FP32);

    size_t spec = g.spectrogram(wav_input, mel_node,
                                 cfg.n_fft, cfg.hop_length, fft_len,
                                 cfg.power, cfg.center, pad,
                                 effective_floor, effective_log,
                                 cfg.dither, cfg.preemphasis,
                                 cfg.remove_dc_offset);
    g.execute();

    const auto& buf = g.get_output_buffer(spec);
    float* out_ptr = static_cast<float*>(g.get_output(spec));
    std::vector<float> result(out_ptr, out_ptr + buf.total_size);

    if (cfg.mel_floor_additive) {
        for (size_t i = 0; i < result.size(); i++)
            result[i] = std::log(result[i] + cfg.mel_floor);
    }
